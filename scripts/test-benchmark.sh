...
# Generate one EndpointSlice for a service. Two slices per service is the shape a
# real cluster has, and without them every backend renders with zero servers —
# which is where the per-object render cost actually lives.
generate_endpointslice() {
    local i="$1"
    local slice="$2"
    local port=$((8080 + (i % 100)))
    ...
    cat <<EOF
          - apiVersion: discovery.k8s.io/v1
            kind: EndpointSlice
            metadata:
              name: bench-svc-${i}-${slice}
              namespace: default
              labels:
                kubernetes.io/service-name: bench-svc-${i}
            addressType: IPv4
            endpoints:
              - addresses: ["10.$((i / 256 % 256)).$((i % 256)).$((slice + 1))"]
                targetRef: { kind: Pod, name: bench-pod-${i}-${slice} }
            ports:
              - port: ${port}
EOF
}
...
generate_fixtures() {
    ...
    echo "        endpoints:"
    for ((i=0; i<count; i++)); do
        generate_endpointslice "$i" 0
        generate_endpointslice "$i" 1
    done
...
create_all_tests_config() {
    ...
    # The chart renders one HAProxyTemplateLibrary per library plus one
    # HAProxyTemplateConfig. Benchmark tests go on the config alone: a test name
    # defined by two objects fails the merge, and a whole-stream yq merge would
    ...
    for step in "${steps_array[@]}"; do
        if [[ "$HTTPROUTE_ONLY" != "true" ]]; then
            add_test_to_config "$cfg_file" "$step" "ingress"
        fi
        if [[ "$INGRESS_ONLY" != "true" ]]; then
            add_test_to_config "$cfg_file" "$step" "httproute"
        fi
    done

    yq eval-all '.' "$libs_file" "$cfg_file" > "$output_config"
    rm -f "$libs_file" "$cfg_file"
}
...
main() {
    ...
    # The bundled chart uses typed resource access, which needs schemas to
    # compile — same auto-wiring as test-templates.sh.
    local schema_dir="${PROJECT_ROOT}/tests/schemas"
