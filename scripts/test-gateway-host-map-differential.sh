...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/haptic-gateway-host-map-diff.XXXXXX")"
...
HAPTIC_GATEWAY_HOST_MAP_BASELINE="$baseline" \
    go test ./pkg/controller/renderer \
        -run '^TestGatewayHostMapColdMatchesDetachedHEADGenerator$' \
        -count=1
