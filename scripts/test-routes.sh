...
# Client-mTLS (haproxy-ingress.github.io/auth-tls-secret): HAProxy
# verifies a client cert against the CA bundle stored in the
# `auth-tls-client-ca` Secret. The chart's ssl.yaml renders the
# `[ca-file <path> verify required]` options into the crt-list line
# for the matching SNI; this test exercises the runtime verification
# path with a valid client cert (200), no client cert (rejected by
# HAProxy at the TLS layer), and a wrong-CA client cert (rejected).
#
# Cert files generated by scripts/start-dev-env.sh's
# `deploy_auth_tls_fixtures` and parked at /tmp/haptic-auth-tls-test/.
test_ingress_auth_tls_secret() {
    if ! should_test "echo-auth-tls"; then
        return 0
    fi

    print_section "Testing Ingress: auth-tls-secret (client mTLS)"

    local cert_dir="${TMPDIR:-/tmp}/haptic-auth-tls-test"
    if [[ ! -f "$cert_dir/client.crt" ]]; then
        err "auth-tls: cert fixtures missing at $cert_dir — start-dev-env.sh did not generate them?"
        return 1
    fi

    local host="auth-tls.localdev.me"
    # Use --connect-to (curl resolves the host itself) rather than --resolve
    # with NODEPORT_IP. In DinD CI, getent hosts often returns only an IPv6
    # address for the docker host; the helper's IPv4-preference filter then
    # falls back to that IPv6 string and curl rejects the unbracketed
    # `--resolve <host>:<port>:<v6:addr>` form (parses the colons as more
    # ports). The existing TLS tests in this file (echo-tls, echo-tls-multi,
    # ssl-passthrough) all use --connect-to for the same reason.
    local connect_arg="${host}:${HTTPS_NODEPORT}:${NODEPORT_HOST}:${HTTPS_NODEPORT}"

    # Case 1: valid client cert signed by the trusted CA → 200.
    # Retry loop handles HAProxy reload race + dataplane warmup.
    local max_retries=8
    local retry_delay=2
    local attempt=1
    ...
    while [[ $attempt -le $max_retries ]]; do
        ...
        response=$(curl -s --max-time 5 \
            ...
            --connect-to "$connect_arg" \
            "https://${host}:${HTTPS_NODEPORT}/" 2>&1)
        curl_exit=$?
        set -e
        if [[ $curl_exit -eq 0 && "$response" == *'"http":'* ]]; then
            ok "auth-tls: valid client cert is accepted, request reaches backend"
            break
        fi
        debug "  Attempt $attempt/$max_retries: curl_exit=$curl_exit response=${response:0:200}"
        if [[ $attempt -eq $max_retries ]]; then
            err "auth-tls: valid client cert path failed after $max_retries attempts (exit=$curl_exit)"
            return 1
        fi
        sleep "$retry_delay"
        attempt=$((attempt + 1))
    ...
    # Case 2: no client cert → HAProxy rejects at TLS handshake.
    # `verify required` makes HAProxy abort the handshake when the
    # client doesn't present a cert; curl returns SSL_ERROR (exit 35
    # or 56 typically) rather than an HTTP response.
    set +e
    response=$(curl -s --max-time 5 \
        --cacert "$cert_dir/ca.crt" \
        --connect-to "$connect_arg" \
    ...
    if [[ $curl_exit -ne 0 ]]; then
        ok "auth-tls: no client cert is rejected at the TLS layer (curl exit=$curl_exit)"
    else
        err "auth-tls: expected TLS rejection without a client cert, got HTTP response: ${response:0:200}"
        return 1
    fi

    # Case 3: client cert signed by an UNTRUSTED CA → HAProxy rejects.
    # Same TLS-layer rejection as case 2 (cert is offered but doesn't
    # validate against the CA bundle).
    set +e
    response=$(curl -s --max-time 5 \
        --cacert "$cert_dir/ca.crt" \
        --cert "$cert_dir/wrong-client.crt" --key "$cert_dir/wrong-client.key" \
        --connect-to "$connect_arg" \
        "https://${host}:${HTTPS_NODEPORT}/" 2>&1)
    curl_exit=$?
    set -e
    if [[ $curl_exit -ne 0 ]]; then
        ok "auth-tls: client cert from untrusted CA is rejected (curl exit=$curl_exit)"
    else
        err "auth-tls: expected rejection of untrusted client cert, got HTTP response: ${response:0:200}"
        return 1
    fi
}
