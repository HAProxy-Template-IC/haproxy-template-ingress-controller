...
| `admissionSubject` | map | The single watched resource under admission review: `{"store": "<watchedResources key>", "namespace": ..., "name": ...}`, or an empty map on reconcile renders, config-proposal renders, and bulk overlays. Combine with `renderMode` to `fail()` only when the violating resource is itself the one being admitted, and fall back to per-route handling otherwise — so one already-present bad resource can't deny unrelated admissions. Read keys with `admissionSubject \| dig("namespace")`. Controller-set; a user's `extraContext.admissionSubject` can't override it. |
