...
| `haproxy-haptic.org/hmac-secret` | ⚠️ Caveat | Names the Secret (data key `secret`) for HMAC request-signature verification (deny `401` on mismatch). The shared key travels (base64) in `haptic-hmac-routes.map`, visible wherever the map is, and the compare isn't constant-time — prefer JWT. Fails closed (`503`) when the Secret is absent. |
| `haproxy-haptic.org/hmac-signed-string` | ✅ Supported | What the signature covers: `body` (default, waits for the request body up to `extraContext.requestBuffering.waitTimeout`) or `path`. |
