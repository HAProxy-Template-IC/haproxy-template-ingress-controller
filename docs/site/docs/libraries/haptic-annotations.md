...
| `haproxy-haptic.org/rate-limit-allowlist` | ⚠️ Caveat | Exempts comma-separated CIDRs from the rate limit; invalid CIDRs fail the render. The only rate-limit setting that's a per-route line in the config, so adding or removing an allowlisted route reloads. |
