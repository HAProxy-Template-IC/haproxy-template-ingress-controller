...
sum by (mode) (rate(haptic_agent_apply_total[5m]))
...
| Removed | Replacement |
|---------|-------------|
| `haptic_dataplane_api_operations_total` | `haptic_agent_apply_total{pod,mode}` — applies, by what the pod did with them |
| `haptic_runtime_fast_path_fires_total` | `haptic_agent_apply_total{mode="runtime"}` |
