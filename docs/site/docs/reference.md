...
| `controller.config.dataplane.reloadVerificationTimeout` | duration | `10s` | How long the agent waits for HAProxy's master to report a reload finished before calling it failed and restoring the last known good file set |
...
| `haproxy.ports.dataplane` | int | `5555` | Single source of truth for the agent's listener, Service, NetworkPolicy, and the controller's connection port |
| `haproxy.ports.agentMetrics` | int | See values.yaml | The agent's Prometheus endpoint, scraped by the bundled PodMonitor |
...
| `haproxy.agent.logLevel` | string | `info` | Log level: `debug`, `info`, `warn`, `error`. At `debug` the agent logs a line per apply with its verdict, the ops it ran and the reload it performed — raise it when diagnosing an apply the controller reports as failing but HAProxy accepts. The stream carries no end-user data; the only client is the controller |
| `haproxy.agent.resources.requests.cpu` | string | See values.yaml | Agent CPU request |
| `haproxy.agent.resources.requests.memory` | string | See values.yaml | Agent memory request (Guaranteed QoS — limits.memory matches) |
| `haproxy.agent.resources.limits.memory` | string | See values.yaml | Agent memory limit |
