...
| `controller.config.templatingSettings.extraContext.serverSlots.increment` | int | `1` | Block size of every backend's reserved server-slot pool. The pool holds `endpoints + max(minFree, ceil(endpoints/4))` slots rounded up to a multiple of this; a full pool grows by a block on the next reload. Per-backend `scale-server-slots` annotations override it |
| `controller.config.templatingSettings.extraContext.serverSlots.minFree` | int | `2` | Minimum spare `disabled` slots kept per backend so scale-ups and rolling updates within the headroom need no reload |
