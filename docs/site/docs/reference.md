...
| `rateLimit.shared.failClosed` | bool | `false` | What happens when the SPOA hub or rate-limit plugin returns no verdict — a reload, a restart, a timeout. Default allows the request: a rate limit protects the origin from abuse, and turning an internal blip into `429` responses sheds legitimate traffic for a reason the client can't act on. The event is still counted (`haptic_denied_total`, and the `rate_limit_unavailable` access-log field), so a limiter silently not limiting stays visible. Set true only where letting a caller past the budget is worse than refusing them — a contractual cap rather than DoS protection |
...
| `controller.config.templatingSettings.extraContext.apiGateway.requestSchemaValidation.defaultFailOpen` | bool | `true` | Default when an Ingress omits `request-schema-fail-open`. An unknown schema id or a missing plugin verdict allows the request and records it: schema validation reduces risk, it doesn't establish identity, so a blip in HAPTIC's own sidecar mustn't refuse callers who did nothing wrong. Missing referenced Kubernetes objects still fail at render time. Set `false` where a malformed body reaching the backend is worse than refusing the caller |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `controller.config.templatingSettings.extraContext.waf.failClosed` | bool | `false` | What happens when the SPOA hub or Coraza plugin returns no verdict — a reload, a restart, a timeout. Default allows the request: a WAF reduces risk, it doesn't establish identity, so a blip in HAPTIC's own sidecar mustn't refuse traffic that did nothing wrong. The event is still counted (`haptic_degraded_total{control="waf"}`, and the `waf_degraded` access-log field), so a WAF silently not inspecting stays visible. Set true only where letting a request past uninspected is worse than refusing it. Requests the WAF did inspect and reject are denied with `403` either way |
...
A [Vector](https://vector.dev) container on every HAProxy pod. It receives the access log over a Unix datagram socket, re-exports HAProxy's and the SPOA hub's Prometheus metrics alongside its own, and derives [per-request metrics](operations/monitoring.md#request-metrics) from the log. See [Access logging](haproxy-deployment.md#access-logging).
...
| `vector.excludeMetrics` | map | eight exclusions, seven on | Named exclusions, each with a `pattern` (a regular expression matched against the **metric name**) and `enabled`. Matching families are dropped before re-export. A map rather than a list so you can disable one without restating the others — Helm replaces a list wholesale, so a list override silently dropped every other exclusion, including any added in a later release. To turn one off: `vector.excludeMetrics.backendAggCheckStatus.enabled=false`. Add your own with any new key. Each entry may also carry `families`, a list of **exact** metric names sent to HAProxy's own scrape filter as `metrics=-<name>` — those never reach Vector's parser, so unlike `pattern` (which filters after the scrape is parsed, and so cuts only what's retained) they cut the per-scrape parse burst. Measured on an 825-backend fleet at the same retained size: peak resident memory 1570 MiB to 1027 MiB, steady 949 MiB to 701 MiB. `pattern` stays authoritative — a family missing from `families` is still dropped, one stage later — and every `families` entry must match its own entry's `pattern`, checked at render time. The defaults drop HAProxy's host-computed maxima, the `agg_check_status` family (reproducible by summing `haproxy_server_check_status`), `agg_server_check_status` (identical to `agg_server_status` unless you use agent checks), and the cache and backup-topology families, which are constant unless you enable those features — together about 23% of the exposition on a large fleet. The compression exclusion ships **off**, because response compression is on by default and those series carry real traffic; turn it on to reclaim them if you disable compression fleet-wide. Two further groups carry `requires: vector.requestMetrics.enabled` and so drop out on their own if you turn that feature off: `haproxy_*_time_average_seconds` (1024-connection rolling averages, superseded by real duration histograms) and `haproxy_{backend,frontend}_http_{requests,responses}_total` (superseded by `<prefix>_requests`, which carries the exact status code plus route, method, host and service). `haproxy_backend_agg_server_status` and `haproxy_server_http_responses_total` are deliberately **kept** — the first is the only remaining free-slot census once `excludeMaintServerMetrics` is on, and the second is per-server, a dimension the request metrics don't carry. An entry may set `requires` itself: a dotted values path that must be truthy for the exclusion to apply. Patterns may not contain quotes, backslashes, or newlines — they're embedded in the rendered Vector config, and an invalid one fails Vector's config load and crash-loops the sidecar |
| `vector.logMetrics` | map | six entries, all on | Metrics derived from the access log, each entry naming a log **field** and how to project it. Several in-path components expose no scrape endpoint of their own — Varnish has none at all, its counters living in shared memory behind `varnishstat` — but the access log already carries their verdict per request, so the metric reads a field that's present anyway. Extraction is a regex over the raw record, never a JSON parse, because the pipeline deliberately doesn't parse access-log records. A map rather than a list so adding an entry doesn't replace the ones the chart ships. Each entry sets `enabled` (required; an entry omitting the flag is refused rather than sitting inert), `field`, `metric`, and `kind`: `enum` emits a counter tagged with the field's value and must list its accepted `values`, so an unexpected value can't invent an unbounded label, with `tag` naming the label (default `value`); `numeric` emits a counter incremented **by** the value. An entry may also set `requires`, a dotted values path that must be truthy — a metric whose emitter is switched off would otherwise cost a regex on every record. Field, metric, tag, and value names are validated at render time, since they're embedded in the rendered Vector config and an invalid one crash-loops the sidecar. Three shipped entries cover cache hit rate, object age, and why responses weren't stored, gated on `cache.varnish.enabled`. Three more carry no `requires` and so are always active: `rateLimitDegraded` and `wafDegraded` (requests allowed because a non-identity control couldn't be consulted) and `deniedBy` (an enum over the deny reasons) |
...
| `vector.podMonitor.enabled` | bool | `false` | Create a `PodMonitor` for Vector's endpoints. Requires the prometheus-operator CRDs. Enabling it replaces the two endpoints the spoa-hub `PodMonitor` declares, and lets `spoaHub.hub.metricsAddr` stay loopback-only. It declares `vector-metrics` always, and `vector-sizes` while a `requestMetrics` size family is enabled; both take the settings below |
