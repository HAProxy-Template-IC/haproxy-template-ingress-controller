...
3. **Authoritative render validation** (leader and proposal paths). The `HAProxyTemplateConfig` has **no** admission webhook — [ADR-0016](adr/0016-one-config-kind-many-instances-pre-rollout-validation.md) removed it, because admission sees one object at a time and a configuration is a set. The leader instead validates the merged set. Watched-resource admission and HTTP-store promotion run `haproxy -c` and every configured rendered-output validator through the proposal pipeline. Reconciliation runs rendered-output validators in its pipeline and `haproxy -c` in the leader-side render gate. Built-in and protocol-v1 external checks execute on every occurrence; a repeated content checksum doesn't reuse a verdict. [ADR-0020](adr/0020-authoritative-render-validation-pipeline.md) defines this invariant.
