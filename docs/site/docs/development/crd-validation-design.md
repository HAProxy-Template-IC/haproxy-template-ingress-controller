...
4. **Deployment validation** (Dataplane API). The API validates the submitted configuration again before activating it. This is defence in depth, not a replacement for the controller-side gate: no invalid render may publish a success event or reach deployment.
