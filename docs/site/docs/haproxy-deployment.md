...
| `service` | The Kubernetes Service behind the chosen backend, as `namespace/name`. Set by the backend that served the request, so it never depends on the backend *name* — a generated identifier that Ingress and Gateway API build differently. Empty when HAProxy answered the request itself |
