...
each is rendered as its own `HAProxyTemplateLibrary`, and a single
`HAProxyTemplateConfig` references them in order via `spec.libraryRefs`
([ADR-0017](0017-template-library-kind.md)); the controller merges the set. Read the loader pipeline below as ending at "strip `_helm_load`" rather than
