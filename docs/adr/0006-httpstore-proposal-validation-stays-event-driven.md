...
- The cost is real: explicit request/batch correlation plus three event types kept alive. We accept that cost in exchange for the non-blocking refresh loop.
...
- The manual batch correlation in `httpstore.handleProposalValidationCompleted` reinvents request/response correlation. `EventBus.Request()` is the wrong tool (it's synchronous-blocking); what's missing from `pkg/events` is an *async* correlation primitive — "publish a request, react to its response on my own event loop." A future change introducing such a primitive would simplify `httpstore` without touching this ADR's decision.
