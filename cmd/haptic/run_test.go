...
// An empty or whitespace-only env value must fall through to the default
// rather than to no config at all, which would leave the controller with
// nothing to watch.
func TestResolveConfigName(t *testing.T) {
	t.Setenv("CRD_NAME", "")
	assert.Equal(t, defaultCRDName, resolveConfigName(""))

	t.Setenv("CRD_NAME", "   ")
	assert.Equal(t, defaultCRDName, resolveConfigName(""))

	t.Setenv("CRD_NAME", "from-env")
	assert.Equal(t, "from-env", resolveConfigName(""))

	// Surrounding whitespace would otherwise reach GetResource and surface as a
	// confusing not-found.
	t.Setenv("CRD_NAME", "  padded-env  ")
	assert.Equal(t, "padded-env", resolveConfigName(""))

	// The flag wins over the env var.
	assert.Equal(t, "from-flag", resolveConfigName("from-flag"))
	assert.Equal(t, "from-flag", resolveConfigName("  from-flag  "))
}
