...
func planMismatches(t *testing.T, config string, mapContents map[string]string, plan *renderplan.Plan) []string {
	...
	if corpusDeclaresBackends(plan) {
		mismatches = append(mismatches, backendSetMismatches(parsed, plan)...)
	}
...
// corpusDeclaresBackends reports whether the render described any backend as
// data. The bundled chart's macros do not call planRegistry.Backend yet, so
// their plans declare none and comparing the parsed backend set against an
// empty one would fail on every fixture. Everything else — the per-backend
// comparison, the section partition, the map entries — is exact either way, and
// the synthetic cases below prove the comparison bites.
func corpusDeclaresBackends(plan *renderplan.Plan) bool {
	return len(plan.Backends) > 0
}
...
func parseBackendServers(t *testing.T, config string) map[string][]parsedServer {
	t.Helper()
	p, err := cnparser.New()
	...
	backends := make(map[string][]parsedServer, len(sections))
	for _, name := range sections {
		backends[name] = nil
		data, err := p.Get(cnparser.Backends, name, "server")
		if err != nil {
		...
		servers := make([]parsedServer, 0, len(lines))
		for i := range lines {
			servers = append(servers, parsedServer{
				Name:    lines[i].Name,
				Address: lines[i].Address,
				Options: serverOptions(&lines[i]),
			})
		}
...
func serverOptions(server *cntypes.Server) map[string]string {
	options := make(map[string]string, len(server.Params))
	for _, param := range server.Params {
...
func backendSetMismatches(parsed map[string][]parsedServer, plan *renderplan.Plan) []string {
	var mismatches []string
...
func declaredServerMismatches(parsed map[string][]parsedServer, plan *renderplan.Plan) []string {
	var mismatches []string
	for _, name := range sortedKeys(plan.Backends) {
		record := plan.Backends[name]
		emitted, found := parsed[name]
		if !found {
			mismatches = append(mismatches, fmt.Sprintf("backend %q is in the plan but not in the config", name))
			continue
		}
		if len(record.Servers) != len(emitted) {
			mismatches = append(mismatches, fmt.Sprintf("backend %q declares %d servers, the config has %d",
				name, len(record.Servers), len(emitted)))
			continue
		}
		for i := range record.Servers {
			mismatches = append(mismatches, serverMismatches(name, &record.Servers[i], &emitted[i])...)
		}
	}
	return mismatches
}
...
func serverMismatches(backend string, declared *renderplan.Server, emitted *parsedServer) []string {
	...
	if want, got := optionalValue(declared.Weight != 0, fmt.Sprint(declared.Weight)), emitted.Options["weight"]; want != got {
...
func optionalValue(present bool, value string) string {
	if !present {
		return ""
	}
	return value
...
// The corpus guard exists only because the bundled chart does not declare
// backends yet. This pins when it lifts: the first macro that calls
// planRegistry.Backend turns the backend-set comparison on for the whole corpus.
func TestCorpusDeclaresBackendsFlipsWithTheFirstBackend(t *testing.T) {
	_, empty := renderSynthetic(t, "global\n    daemon\n", nil)
	assert.False(t, corpusDeclaresBackends(empty))

	_, declared := renderSynthetic(t, backendTemplate(twoServerRecord, twoServerText), nil)
	require.True(t, corpusDeclaresBackends(declared))

	// With the guard lifted, a backend the plan does not know about is a
	// failure rather than an accepted gap.
	config := "global\n    daemon\nbackend be_undeclared\n    server SRV_1 10.0.0.1:8080\n"
	assert.Contains(t, strings.Join(backendSetMismatches(parseBackendServers(t, config), declared), "\n"),
		`backend "be_undeclared" is in the config but not in the plan`)
}
