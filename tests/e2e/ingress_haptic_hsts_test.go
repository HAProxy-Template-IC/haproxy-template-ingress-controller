...
// TestHapticHSTS covers haproxy-haptic.org/hsts and the associated hsts-*
// sub-annotations: the chart must surface a Strict-Transport-Security header on
// responses with the configured max-age plus optional includeSubDomains and
// preload directives.
//
// HSTS is only meaningful — and the chart only emits the header — on HTTPS
// responses (the `{ ssl_fc }` ACL gates the directive). Hence the test creates a
...
func TestHapticHSTS(t *testing.T) {
	...
	RunSimpleIngressTest(t, SimpleIngressTest{
		...
		Annotations: map[string]string{
			...
			"haproxy-haptic.org/hsts-max-age":            "31536000",
			"haproxy-haptic.org/hsts-include-subdomains": "true",
			"haproxy-haptic.org/hsts-preload":            "true",
		},
