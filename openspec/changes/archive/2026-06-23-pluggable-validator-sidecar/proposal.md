> **Update 2026-05-04 — design correction.** The CRD field, cache key, and protocol-doc framing in this proposal drifted from the originally-discussed design (controller-side glob routing, per-(validator, file, content) cache, validator-program opaqueness, persistent keep-alive connections, parallel dispatch). The corrected design lives in `specs/pluggable-validator-sidecar/spec.md` in this same change directory, and `docs/development/validator-protocol.md` is the authoritative wire-protocol contract. The text below is preserved as historical context for what landed in MR !887.
