...
Each enabled library SHALL be rendered as its own `HAProxyTemplateConfig` object; the CONTROLLER merges them, in the fixed order base, ssl, ingress, gateway, ingress-annotations-compat, haptic-annotations, haproxytech, haproxy-ingress, nginx-ingress, spoa-hub, vector, values.yaml. The merge SHALL use the same primitive as sprig's mustMergeOverwrite (mergo with override), so the controller-side result matches what a chart-side merge would have produced; see ADR-0014. The base, ssl, ingress, and ingress-annotations-compat libraries live as files (or split-library directories) under `libraries/`; the gateway, haproxytech, haproxy-ingress, and nginx-ingress libraries are conditional SUBCHARTS (`charts/haptic/charts/<name>/`) referenced as `subchart:<name>` entries in `haptic.libraryFiles`. A disabled subchart is pruned from the release Secret, so its `.Subcharts.<name>` is absent, the entry is skipped, and no object is rendered for it. Later libraries SHALL override earlier ones for the same keys. Each library SHALL be independently enableable via values.yaml under controller.templateLibraries.<name>.enabled. The gateway library SHALL additionally require Gateway API CRDs to be present (Capabilities.APIVersions.Has "gateway.networking.k8s.io/v1/GatewayClass"). The regex-last path matching order is NOT a separate library; it is selected via controller.config.templatingSettings.extraContext.routing.regexMatchOrder ("default" or "last") which swaps a snippet inside base.yaml at Helm render time.
...
> **Superseded — pod-named servers (ADR-0011).** The numbered slot pool (`SRV_1`…`SRV_N`), the `192.0.2.1:1` placeholder padding, `serverSlotsValue`, and slot preservation described in this and the following slot-allocation requirements no longer match the implementation: a backend renders exactly its current pods as `server <pod> <ip>:<port> guid srv:<be>:<pod>` lines, added and removed over the runtime API with no reserved slots. This spec text is pending an OpenSpec change to sync it to the pod-named model; see `docs/site/docs/libraries/reload-free.md`.
