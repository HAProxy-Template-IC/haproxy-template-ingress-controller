...
- Both ingress libraries wire the `auth-headers-pass` annotations (`nginx.ingress.kubernetes.io/auth-response-headers` and `haproxy-ingress.github.io/auth-headers-pass`); haproxy-ingress additionally wires `auth-headers-fail`. The per-ingress comma-list of header names lands in the new `auth-extract-headers.map` (as the union of pass + fail headers per ingress) and surfaces in the SPOE check-auth message's `extract_headers` arg. The plugin v0.3.0+ extracts those headers from every reply path (2xx, 3xx, 4xx, 5xx) and publishes each as a transaction variable `txn.hub.external_auth.<name>` (lowercased, dashes → underscores). Two new `frontend-spoe-set-pass-headers-*` and `frontend-spoe-set-fail-headers-*` extension points emit one HAProxy directive per unique header: `http-request set-header` for pass (gated on `allowed -m bool`, fires on backend forwarding) and `http-response set-header` for fail (gated on `var(txn.auth_url) -m found` and `!allowed`, fires on the deny response so e.g. `WWW-Authenticate` reaches the client to drive a Bearer challenge). Per-route gating happens implicitly via the plugin's per-route `extract_headers` SPOE arg — routes that didn't ask for a header have its txn var unset, so the `var ... -m found` gate skips them.
- haproxy-ingress library wires the `haproxy-ingress.github.io/auth-tls-cert-secret` annotation: per-route reference to a `kubernetes.io/tls` Secret whose `tls.crt` / `tls.key` flow into the SPOE check-auth message's `client_cert` / `client_key` args, enabling per-ingress mTLS to the auth service (plugin v0.3.0+). The chart passes the already-base64-encoded PEM from the Secret data field verbatim into the new `auth-tls-client-cert.map` / `auth-tls-client-key.map` (multi-line PEM would otherwise break HAProxy map parsing); the spoa-hub frontend chains a `,b64dec` converter on lookup so the SPOE message args land as raw PEM. Missing Secret, missing data fields, or a host-less rule each fail loud at template-render time — silently dropping mTLS on a route the operator marked for it would let traffic out without the expected client cert. nginx-ingress's `auth-tls-secret` is intentionally not wired (haproxy-ingress flavour covers the use case end-to-end and the templating cost is identical).
