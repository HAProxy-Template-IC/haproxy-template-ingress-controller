...
| `ResourceAdmissionTimeout` | Deadline applied around each watched-resource dry-run validation (default `9s`, applied when zero). Keep it below the `ValidatingWebhookConfiguration`'s `timeoutSeconds` so the controller returns a structured decision instead of an HTTP transport failure. |
| `Server` | An already-bound `*pkg/webhook.Server` to adopt instead of creating one. The controller hoists the listener to process lifetime and swaps only the validator table per iteration, so the API server never dials a dead port during reinitialisation. The adopting component must not shut it down. |
| `DryRunValidator` | Interface with a single method: `ValidateDirect(ctx, gvk, namespace, name, object, operation) (allowed bool, reason string, warnings []string)`. Satisfied by `pkg/controller/dryrunvalidator.Component`. Warnings flow through to `AdmissionResponse.Warnings` on both allow and deny paths. A configured webhook rule without this dependency fails registration. |
