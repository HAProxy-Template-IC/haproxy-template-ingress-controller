...
- Publishes: `HAProxyPodsDiscoveredEvent`, `HAProxyPodTerminatedEvent`, `HAProxyPodRejectedEvent` (one per pod that fails admission, labelled `no_pod_ip`, `agent_container_not_running` or `agent_unreachable` — the metrics component turns these into `haptic_haproxy_pods_rejected_total`)
