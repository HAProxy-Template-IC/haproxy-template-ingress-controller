...
- Applies the per-resource `debounceInterval` override (or the 100ms `pkg/k8s/types.DefaultDebounceInterval` when empty; `"0"` disables debouncing so every change fires immediately) to the watcher's leading-edge refractory window
