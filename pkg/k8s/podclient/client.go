...
type podLoopbackGetter func(context.Context, string, string, int64) ([]byte, error)

const loopbackRequestTimeout = 15 * time.Second

// Client reaches a pod-local HTTP endpoint through Kubernetes port-forwarding.
type Client struct {
	config        *rest.Config
...
// New creates a client for ready pods matching a label selector.
func New(
	config *rest.Config,
	clientset kubernetes.Interface,
	namespace string,
...
func (c *Client) Get(ctx context.Context, path string) ([]byte, error) {
	...
	for offset := uint64(0); offset < nameCount; offset++ {
		...
		body, err := c.getFromPod(requestCtx, name, path, 0)
		cancel()
		if err == nil {
			return body, nil
		}
...
func (c *Client) portForwardGet(config *rest.Config) podLoopbackGetter {
	return func(ctx context.Context, podName, path string, maxBytes int64) ([]byte, error) {
		...
		return getForwardedLoopback(ctx, localPort, path, maxBytes)
	}
}

func (c *Client) newPortForwarder(
	config *rest.Config,
	podName string,
...
func getForwardedLoopback(ctx context.Context, localPort uint16, path string, maxBytes int64) ([]byte, error) {
	...
	body, err := readResponse(response.Body, maxBytes)
	if err != nil {
		return nil, fmt.Errorf("read loopback response: %w", err)
	}
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices {
		return nil, fmt.Errorf("loopback endpoint returned HTTP %d", response.StatusCode)
	}
	return body, nil
}
