...
func TestCachedStoreRevisionAPIUsesPinnedRoot(t *testing.T) {
	...
	require.NotZero(t, resourceStore.RevisionSource())
	require.NotEmpty(t, resourceStore.ListRevision())
	require.NotEmpty(t, resourceStore.GetRevision("green", "target"))
	require.NotEmpty(t, resourceStore.IdentityRevision("default", "target"))
	listItems, listSequence, listErr := resourceStore.ListSnapshot()
	require.NoError(t, listErr)
	require.Len(t, listItems, 1)
	require.Equal(t, uint64(2), listSequence)
	_, changes, complete := resourceStore.ChangesSince(0)
	require.True(t, complete)
	require.Len(t, changes, 2)
	items, revision, sequence, snapshotErr := resourceStore.GetSnapshot("green", "target")
