...
// WrapSlice is the slice-shaped variant of [WrapInto]. It converts a
// []map[string]any (the StoreWrapper snapshot shape) into a
// []*T-typed slice — actually a reflect.Value whose dynamic type is
// reflect.SliceOf(reflect.PointerTo(elem)). Caller hands it to Scriggo
// via the typed-nil-pointer global pattern documented in
// pkg/templating/globals.go: the declaration ships the type, the
// runtime ships the value.
//
// Element-level errors stop the conversion — partial slices would be
// worse than no slice, because the chart would emit a partial routing
// table and 404 the missing routes. The watcher's own validation
// catches structural problems earlier; getting here with a malformed
// resource means something deeper is wrong.
func WrapSlice(items []any, elem reflect.Type) (reflect.Value, error) {
	if elem == nil {
		return reflect.Value{}, fmt.Errorf("typegen: WrapSlice called with nil element type")
	}
	ptrElem := reflect.PointerTo(elem)
	sliceType := reflect.SliceOf(ptrElem)
	out := reflect.MakeSlice(sliceType, 0, len(items))

	for i, item := range items {
		m, ok := item.(map[string]any)
		if !ok {
			return reflect.Value{}, fmt.Errorf("typegen: WrapSlice item %d: want map[string]any, got %T", i, item)
		}
		v, err := WrapInto(m, elem)
		if err != nil {
			return reflect.Value{}, fmt.Errorf("typegen: WrapSlice item %d: %w", i, err)
		}
		// WrapInto returns a settable Value of type T; we want *T so
		// Scriggo's range over []*T yields pointers and templates can
		// keep using the same variable shapes they use today (the
		// unstructured maps were passed by interface, which is
		// likewise pointer-flavoured for any non-scalar payload).
