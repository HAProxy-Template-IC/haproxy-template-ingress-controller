...
trap stop_hap EXIT
start_hap "$VER" "$SPIKE_DIR/mapenv4" || exit 1
...
python3 - "$OUT_DIR/b4big3000.txt" <<'PY'
import sys
with open(sys.argv[1], 'w') as f:
    ...
    for i in range(2999):
        f.write("/k%d val%d\n" % (i, i))
PY
echo "-- payload file: $(wc -l < "$OUT_DIR/b4big3000.txt") lines, $(wc -c < "$OUT_DIR/b4big3000.txt") bytes"

LOOPLOG="$OUT_DIR/atomicity4-$VER.log"
: > "$LOOPLOG"
(
  end=$((SECONDS+20))
...
T1=$(date +%s.%N)
payfile "add map @$V3 $VERM" "$OUT_DIR/b4big3000.txt" > "$OUT_DIR/b4bigadd-$VER.out" 2>&1
T2=$(date +%s.%N)
...
echo "-- payload add response bytes: $(wc -c < "$OUT_DIR/b4bigadd-$VER.out"); first lines:"
head -4 "$OUT_DIR/b4bigadd-$VER.out" | sed 's/^/    /'
...
python3 - "$OUT_DIR/b4big3000b.txt" <<'PY'
import sys
with open(sys.argv[1], 'w') as f:
    f.write("/v NAIVEGEN\n")
    for i in range(2999):
...
payfile "add map $VERM" "$OUT_DIR/b4big3000b.txt" >/dev/null 2>&1
T1=$(date +%s.%N)
...
python3 - "$OUT_DIR/b4big30000.txt" <<'PY'
import sys
with open(sys.argv[1], 'w') as f:
    f.write("/v BIGGEN\n")
    for i in range(29999):
        f.write("/m%d val%d\n" % (i, i))
PY
echo "   payload bytes: $(wc -c < "$OUT_DIR/b4big30000.txt")"
T0=$(date +%s.%N)
PREP4=$(cli "prepare map $VERM"); V4=$(echo "$PREP4" | grep -oE '[0-9]+' | head -1)
payfile "add map @$V4 $VERM" "$OUT_DIR/b4big30000.txt" >/dev/null 2>&1
