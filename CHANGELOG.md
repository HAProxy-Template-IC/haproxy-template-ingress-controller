...
- `resourceapplier` now supports a partial-ownership mode. When a rendered resource carries the `haproxy-haptic.org/ownership: partial` annotation, the applier (a) skips injecting the `managed-by` label, (b) does not track the resource in its orphan-cleanup set (orphan disappearance never DELETEs a partial-mode resource), and (c) strips the annotation from the SSA payload before applying. Field-manager-aware list-map merging on the apiserver handles per-entry ownership — chart-owned and haptic-owned entries on the same resource (e.g. `Service.spec.ports` keyed by `(port, protocol)`) coexist without conflict. Enables the chart's gateway library to write Gateway-listener `gw-<port>-<proto>` ports onto the helm-owned `haptic-haproxy` Service via Server-Side Apply rather than emitting a separate Service.
