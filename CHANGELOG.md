...
- Shared response cache: an opt-in, chart-deployed Varnish tier (`controller.cache.varnish.enabled`) that HAProxy shards by URL with bounded-load consistent hashing, so the cache is shared across the whole HAProxy fleet (loopback origin — on a miss Varnish fetches back through HAProxy, which routes to the app). Deployed as a StatefulSet (or Deployment) + headless Service + static VCL via the controller's `k8sResources` (Server-Side Apply, GC'd on uninstall). Per-route control via native annotations: `cache-enable`, `cache-ttl`, `cache-key` (vary by consumer/header/cookie/query/src — making it safe to cache authenticated content per consumer), `cache-exclude-content-types`, `cache-exclude-paths`, and `cache-max-object-size`. Cached responses carry an `X-Cache: HIT`/`MISS` header. The tier can autoscale via an optional HorizontalPodAutoscaler (`controller.cache.varnish.autoscaling`) — bounded-load consistent hashing keeps per-shard load even, so average CPU is a sound signal; a generous scale-down window guards cache warmth. The controller Role grants `statefulsets`/`configmaps` (and `horizontalpodautoscalers` when autoscaling is on) only when the tier is enabled.
