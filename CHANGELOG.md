...
- Reusable Coraza WAF policies for native Ingress annotations: HAPTIC administrators can define bounded policies in chart values or exact, RBAC-protected cross-namespace ConfigMaps, while Ingress authors select one with `haproxy-haptic.org/waf-policy`. Policies support request-body modes, enforcement, structured CRS target exclusions, and advanced SecLang. Optional immutable defaults cannot be removed or replaced by an Ingress; mode, arbitrary rules, vendor compatibility annotations, and raw HAProxy escape hatches are centrally authorized and default-deny where they could weaken enforcement. Shared policies compile once across routes, while native/nginx per-Ingress rule applications have configurable count and byte bounds.
