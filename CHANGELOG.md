...
- Admission timeout values are now configurable as `webhook.timeoutSeconds` (watched resources, default `10`) and `webhook.haproxyTemplateConfig.timeoutSeconds` (prospective config, default `30`); the chart validates Kubernetes' limit and derives one-second-shorter controller deadlines automatically.
