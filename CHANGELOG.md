...
- When two Ingresses claim the same host, path, and path type, the route is now resolved deterministically to the older Ingress (by `creationTimestamp`, tiebroken by namespace/name) instead of the previous arbitrary render-order winner — matching ingress-nginx's oldest-wins, so an Ingress that was there first is never hijacked by a later conflicting one. `Prefix` and `ImplementationSpecific` share a routing slot and resolve together; `Exact` is separate. An nginx canary Ingress no longer competes for its main's base route — the main Ingress owns the route and the canary only overlays its traffic split (previously the canary could win the base map by render order, sending non-canary traffic to the canary backend).
