...
- The dormant event-driven webhook-cert-rotation pipeline (the `certloader` component, its cert events, the webhook-cert watcher, and the `haptic_webhook_cert_expiry_timestamp_seconds` metric) — it was never wired in and never rotated certs; webhook-cert hot-reload (see Added) replaces it.
